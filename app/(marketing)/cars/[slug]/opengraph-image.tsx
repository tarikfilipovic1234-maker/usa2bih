...
export default async function VehicleOg({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", background: "#0a0c10", fontFamily: "sans-serif" }}>
        {img && (
        ...
        <div
          style={{
            ...
            padding: 64,
            color: "#eef1f6",
            background: "#171b24", border: "1px solid #333a48",
          }}
        >
          <div style={{ display: "flex", fontSize: 28, fontWeight: 700 }}>
            USA<span style={{ color: "#56a8f0" }}>2</span>BIH
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ fontSize: 26, color: "#56a8f0", fontWeight: 600 }}>{String(vehicle.year)}</div>
            <div style={{ fontSize: 60, fontWeight: 800, lineHeight: 1.05, letterSpacing: -1 }}>
              {`${vehicle.make} ${vehicle.model}`}
            </div>
            <div style={{ fontSize: 28, color: "#868f9e" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
            ...
            <div style={{ fontSize: 26, color: "#868f9e" }}>auction price</div>
          </div>
        </div>
      </div>
    ),
